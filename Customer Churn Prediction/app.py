import streamlit as st
import joblib
import numpy as np

scaler=joblib.load("scaler.pkl")
model=joblib.load("model.pkl")

st.title("Churn Prediction App")
st.divider()
st.write("Please enter the values and hit the predict")
st.divider()
age=st.number_input("Enter age", min_value=10,max_value=100,value=30)
gender=st.selectbox("Enter the gender",["Male","Female","Other"])
tenure=st.number_input("Enter Tenure",min_value=0,max_value=100,value=10)
monthly_charges=st.number_input("Enter monthly charges",min_value=30,max_value=150)

st.divider()

predictbutton=st.button("Predict")

if predictbutton:
    gender_select =1 if gender=="Female" else 0
    X= [age,gender_select,tenure,monthly_charges]
    X1= np.array(X)
    X_array=scaler.transform([X1])
    prediction=model.predict(X_array)[0]
    predicted="Yes" if prediction ==1 else "No"
    st.balloons()
    st.write(f"Predicted : {predicted}")
else:
    st.write("Please enter the value and use predict button")